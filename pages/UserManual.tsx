...
const UserManual: React.FC = () => {
  ...
  const userManuals = useMemo(() => DOC_FILES.filter((doc) => doc.category === 'user_manual'), []);
  const filtered = useMemo(() => {
    return userManuals.filter((item) => item.filename.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="mb-12">
        ...
        <p className="text-slate-600 text-lg">按文件名搜索并打开用户手册。</p>
      ...
      <div className="mb-8 relative">
        <input
          type="text"
          placeholder="搜索 User Manual 文件名"
      ...
      <div className="mb-4 text-sm text-slate-500">共找到 {filtered.length} 份用户手册</div>
