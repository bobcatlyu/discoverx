...
const OthersDoc: React.FC = () => {
  ...
  const filtered = miscDocs.filter(doc => doc.title?.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
        <div className="p-20 text-center">
          ...
          <p className="text-slate-400 text-lg mb-2">其他资料文件即将上传</p>
          <p className="text-slate-500 text-sm">Other document files will be available soon</p>
        </div>
      </div>
    </div>
  );
